...
public class NoticiaController extends GenericController<Noticia, Integer> {
    ...
    public ResponseEntity<List<Noticia>> listarNoticiasPorCategoria(
            @PathVariable(value = "id_categoria") Integer id_categoria) {
        Categoria categoria = categoriaRepository.findById(id_categoria).get();
        return ResponseEntity.ok((List<Noticia>) noticiaRepository.findAllByCategoria(categoria));
    }

    @RequestMapping(value = "findTopByCategoriaIdOrderByDataPublicacaoDesc/{id_categoria}", method = RequestMethod.GET)
    public ResponseEntity<Noticia> findOneByUltimaNoticiaByCategoria(
            @PathVariable(value = "id_categoria") Integer id_categoria) {
        return ResponseEntity
                .ok((Noticia) noticiaRepository.findTopByCategoriaIdOrderByDataPublicacaoDesc(id_categoria));
    }
    ...
    @RequestMapping(value = "findTopByDestaqueTrueOrderByDataPublicacaoDesc", method = RequestMethod.GET)
    public ResponseEntity<Noticia> findTopByDestaqueTrueOrderByDataPublicacaoDesc() {
        return ResponseEntity.ok((Noticia) noticiaRepository.findTopByDestaqueTrueOrderByDataPublicacaoDesc());
    }
